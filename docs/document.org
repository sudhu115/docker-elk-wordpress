...
**** Wordpress configuration
     ...
     configuring it. For wordpress to work host machine's port 8081 is
     mapped to port 80 of docker container. Wordpress container is
...
* Filebeat log collector
  I tried configuring logstash with filebeat input but it was not
  working. I configured my logstash.conf as follows:
  #+BEGIN_EXAMPLE
  input {
  tcp {
     port => 5000
  ...
  I generated the ip based ssl certificate also. In the logstash
  container configuration seems to be fine.
  #+BEGIN_EXAMPLE
  $ service logstash configtest                                                                                                
  Configuration OK
  #+END_EXAMPLE
  
  The error is occurring at the wordpress container. I configured
        filebeat plugin inside the container to forward logs to
        logstash container, but it is having some issues which I am
        not able to resolve right now.
  
  Configuration of /etc/filebeat/filebeat.yml is as follows:
  #+BEGIN_EXAMPLE
  filebeat:
  prospectors:
  -
  ...
  The ip mentioned in the hosts in ubuntu host machine ip, not the
  container's ip. I was trying if it works. It does not work with
  client machine's ip either. When I restart filebeat service it is
  throwing ssl connection error:
  #+BEGIN_EXAMPLE
  Restarting Sends log files to Logstash or directly to Elasticsearch.: filebeat2016/06/22 02:24:09.861547 transport.go:125: ERR SSL client failed to connect with: EOF
  .
  #+END_EXAMPLE
